...
static void
calf_tube_snapshot (GtkWidget *widget, GtkSnapshot *snapshot)
{
    ...
    int sx = width - (ox * 2), sy = height - (oy * 2);
...
//        }
        ...
        cairo_set_source_surface (cache_cr, image, width / 2 - sx / 2 + inner, height / 2 - sy / 2 + inner);
        cairo_paint (cache_cr);
...
static void
calf_tube_measure (GtkWidget *widget, GtkOrientation orientation, int for_size,
...
static void
calf_tube_size_allocate (GtkWidget *widget, int width, int height, int baseline)
{
    g_assert(CALF_IS_TUBE(widget));
...
calf_tube_class_init (CalfTubeClass *klass)
{
    ...
    widget_class->snapshot      = calf_tube_snapshot;
    widget_class->measure       = calf_tube_measure;
...
calf_tube_init (CalfTube *self)
{
    ...
    switch(self->direction) {
        case 1:
            switch(self->size) {
                case 1:
                    gtk_widget_set_size_request(widget, 82, 130);
                    break;
                default:
                case 2:
                    gtk_widget_set_size_request(widget, 130, 210);
                    break;
            }
            break;
        default:
        case 2:
            switch(self->size) {
                case 1:
                    gtk_widget_set_size_request(widget, 130, 82);
                    break;
                default:
                case 2:
                    gtk_widget_set_size_request(widget, 210, 130);
                    break;
            }
            break;
    }
