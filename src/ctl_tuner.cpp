...
static void
calf_tuner_snapshot (GtkWidget *widget, GtkSnapshot *snapshot)
{
    ...
    if ( tuner->background == NULL ) {
        ...
        tuner->background = cairo_image_surface_create(CAIRO_FORMAT_ARGB32, width, height);
    ...
    if (tuner->note) {
        ...
        // octave
        char octn[20];
        snprintf(octn, sizeof(octn), "%d", oct);
        ...
        char mnotev[32];
        snprintf(mnotev, sizeof(mnotev), "%d", tuner->note);
        const char * centst = "Cents: ";
        char centsv[32];
        snprintf(centsv, sizeof(centsv), "%.4f", tuner->cents);
...
static void
calf_tuner_measure (GtkWidget *widget,
...
static void
calf_tuner_size_allocate (GtkWidget *widget, int width, int height, int baseline)
{
    g_assert(CALF_IS_TUNER(widget));
...
calf_tuner_class_init (CalfTunerClass *klass)
{
    ...
    widget_class->snapshot      = calf_tuner_snapshot;
    widget_class->measure       = calf_tuner_measure;
