...
static void
calf_line_graph_snapshot (GtkWidget *widget, GtkSnapshot *snapshot)
{
    g_assert(CALF_IS_LINE_GRAPH(widget));
    CalfLineGraph *lg = CALF_LINE_GRAPH(widget);
    ...
    if (lg->recreate_surfaces) {
        // hardcode pad values (GTK4: no xthickness/ythickness)
        lg->pad_x = 1;
        lg->pad_y = 1;
        // In GTK4 widget-local coordinates always start at 0,0
        lg->x = 0;
        lg->y = 0;
    ...
    // window surface (widget-local coordinates: x=0, y=0)
    if (lg->debug) printf("copy realtime->window\n");
    calf_line_graph_copy_surface(c, lg->realtime_surface, 0, 0);
    ...
    if (lg->freqhandles) {
        ...
        calf_line_graph_copy_surface(c, lg->handles_surface, 0, 0);
    }
    ...
    if (lg->use_crosshairs && lg->crosshairs_active && lg->mouse_x > 0
        ...
        calf_line_graph_draw_crosshairs(lg, c, false, 0, 0.5, 5, false, lg->mouse_x - ox, lg->mouse_y - oy, s, 1.5, 0, 0);
    }
...
static void
calf_line_graph_pointer_motion (GtkEventControllerMotion *controller, double x, double y, gpointer data)
{
    ...
    if (handle_hovered != lg->handle_hovered) {
        if (lg->handle_grabbed >= 0 ||
            handle_hovered != -1) {
            gtk_widget_set_cursor(widget, lg->hand_cursor);
            lg->handle_hovered = handle_hovered;
        } else {
            gtk_widget_set_cursor(widget, lg->arrow_cursor);
            lg->handle_hovered = -1;
        }
        lg->handle_redraw = 1;
        calf_line_graph_expose_request(widget, true);
...
static void
calf_line_graph_button_press (GtkGestureClick *gesture, int n_press, double x, double y, gpointer data)
{
...
static void
calf_line_graph_button_release (GtkGestureClick *gesture, int n_press, double x, double y, gpointer data)
{
    GtkWidget *widget = gtk_event_controller_get_widget(GTK_EVENT_CONTROLLER(gesture));
    g_assert(CALF_IS_LINE_GRAPH(widget));
    CalfLineGraph *lg = CALF_LINE_GRAPH(widget);
...
static gboolean
calf_line_graph_scroll (GtkEventControllerScroll *controller, double dx, double dy, gpointer data)
{
    GtkWidget *widget = gtk_event_controller_get_widget(GTK_EVENT_CONTROLLER(controller));
    g_assert(CALF_IS_LINE_GRAPH(widget));
    CalfLineGraph *lg = CALF_LINE_GRAPH(widget);
...
static void
calf_line_graph_enter (GtkEventControllerMotion *controller, double x, double y, gpointer data)
{
    GtkWidget *widget = gtk_event_controller_get_widget(GTK_EVENT_CONTROLLER(controller));
    CalfLineGraph *lg = CALF_LINE_GRAPH(widget);
...
static void
calf_line_graph_leave (GtkEventControllerMotion *controller, gpointer data)
{
    GtkWidget *widget = gtk_event_controller_get_widget(GTK_EVENT_CONTROLLER(controller));
    g_assert(CALF_IS_LINE_GRAPH(widget));
    CalfLineGraph *lg = CALF_LINE_GRAPH(widget);
...
static void
calf_line_graph_measure (GtkWidget *widget, GtkOrientation orientation, int for_size,
...
static void
calf_line_graph_size_allocate (GtkWidget *widget, int alloc_width, int alloc_height, int baseline)
{
    g_assert(CALF_IS_LINE_GRAPH(widget));
    CalfLineGraph *lg = CALF_LINE_GRAPH(widget);
    ...
    int width  = alloc_width;
    int height = alloc_height;
    ...
    if (parent_class->size_allocate)
        parent_class->size_allocate(widget, alloc_width, alloc_height, baseline);
}
...
calf_line_graph_class_init (CalfLineGraphClass *klass)
{
    ...
    widget_class->snapshot      = calf_line_graph_snapshot;
    widget_class->measure       = calf_line_graph_measure;
...
calf_line_graph_init (CalfLineGraph *lg)
{
    ...
    // hardcode pad values (GTK4: no style xthickness/ythickness)
    lg->pad_x                = 1;
    lg->pad_y                = 1;
    lg->force_cache          = true;
    lg->force_redraw         = false;
    ...
    lg->movesurf             = 0;
    lg->generation           = 0;
    lg->arrow_cursor         = gdk_cursor_new_from_name("default", NULL);
    lg->hand_cursor          = gdk_cursor_new_from_name("all-scroll", NULL);
    ...
    g_signal_connect(G_OBJECT(widget), "unrealize", G_CALLBACK(calf_line_graph_unrealize), (gpointer)lg);
    ...
    // Motion controller: handles motion, enter and leave
    GtkEventControllerMotion *motion = GTK_EVENT_CONTROLLER_MOTION(gtk_event_controller_motion_new());
    g_signal_connect(motion, "motion", G_CALLBACK(calf_line_graph_pointer_motion), NULL);
    g_signal_connect(motion, "enter",  G_CALLBACK(calf_line_graph_enter),          NULL);
    g_signal_connect(motion, "leave",  G_CALLBACK(calf_line_graph_leave),          NULL);
    gtk_widget_add_controller(widget, GTK_EVENT_CONTROLLER(motion));

    // Click gesture: handles button press and release
    GtkGestureClick *click = GTK_GESTURE_CLICK(gtk_gesture_click_new());
    g_signal_connect(click, "pressed",  G_CALLBACK(calf_line_graph_button_press),   NULL);
    g_signal_connect(click, "released", G_CALLBACK(calf_line_graph_button_release), NULL);
    gtk_widget_add_controller(widget, GTK_EVENT_CONTROLLER(click));

    // Scroll controller
    GtkEventControllerScroll *scroll = GTK_EVENT_CONTROLLER_SCROLL(
        gtk_event_controller_scroll_new(GTK_EVENT_CONTROLLER_SCROLL_BOTH_AXES));
    g_signal_connect(scroll, "scroll", G_CALLBACK(calf_line_graph_scroll), NULL);
